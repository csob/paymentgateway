...
5. for others operations use following params ...
	...
	payment oneclick init
	  ...
	  run.bat -m PAYMENT_ONECLICK_INIT -r config/payment-oneclick-base.json
	  run.sh  -m PAYMENT_ONECLICK_INIT -r config/payment-oneclick-base.json

	payment oneclick start

	  run.bat -m PAYMENT_ONECLICK_START -p <pay-id-from-previous-payment-oneclick-init-call>
	  run.sh  -m PAYMENT_ONECLICK_START -p <pay-id-from-previous-payment-oneclick-init-call>
