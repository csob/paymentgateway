...
public class PaymentCloseMode implements RunMode {
	...
	public void proc(ArgsConfig aConfig) {
		...
		try {
			...
			if (aConfig.amount != null) {
				LOG.info("using totalAmount {} for payment/close", aConfig.amount);
			}
			PayRes res = nativeApiService.paymentClose(aConfig.payId, aConfig.amount);
