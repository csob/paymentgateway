...
namespace CsobGatewayClientExample.Communication
{
    public class GatewayClient
    {
        ...
        public async Task<ClientResponse> CallOneClickInitAsync()
        {
            ...
            return await CreatePostRequestAsync("oneclick/init", request);
        }
        ...
        public async Task<ClientResponse> CallOneClickStartAsync(string payId)
        {
            ...
            return await CreatePostRequestAsync("oneclick/start", request);
        }

        public async Task<ClientResponse> CallOneClickEchoAsync()
        {
            PayOneclickInitReq requestJson;
            using (var file = File.OpenText(Constants.PaymentOneclickBaseFilePath))
                requestJson = JsonConvert.DeserializeObject<PayOneclickInitReq>(file.ReadToEnd());

            var request = new OneclickEchoReq()
            {
                OrigPayId = requestJson.OrigPayId
            };

            FillAndSign(request);

            return await CreatePostRequestAsync("oneclick/echo", request);
        }
